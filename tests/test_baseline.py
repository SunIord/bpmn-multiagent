"""
Teste do baseline monolítico.

Lê as fixtures de entrada, gera BPMN via run_monolithic() para cada
processo e salva os XMLs em data/outputs/baseline/. Em seguida, valida
a boa formação de cada XML com lxml e imprime um relatório de observações.
...
def check_xml(xml_str: str) -> dict[str, object]:
    ...
    try:
        ...
        for tag_name, _ in BPMN_TAGS_TO_CHECK:
            # Tenta com namespace e sem namespace (modelos menos disciplinados)
        ...
        result["erro"] = "lxml não instalado — instale com: pip install lxml"
...
def main() -> None:
    print("=" * 60)
    print("  BASELINE MONOLÍTICO — Geração de BPMN (one-shot)")
    print("=" * 60)
    ...
    # 2. Processa cada descrição
    for i, descricao in enumerate(processos, start=1):
        output_path = OUTPUT_DIR / f"processo{i}_baseline.xml"
        print(f"[{i}/{len(processos)}] Gerando BPMN para o Processo {i}...")
        print(f"       Descrição: {descricao[:80].replace(chr(10), ' ')}...")

        try:
            xml: str = run_monolithic(descricao)
        except Exception as exc:  # noqa: BLE001
            print(f"  [ERRO] Falha ao gerar XML: {exc}")
            continue

        # 3. Salva o XML gerado
        output_path.write_text(xml, encoding="utf-8")

        # 4. Valida e imprime relatório
